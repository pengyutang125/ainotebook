...
public class CompilationUnitBuilder {
    ...
    public CompilationUnitBuilder(JavaSrcRepository repository, Qualifier qualifier, String src) {
    ...
    public void startType(int line, String name, Type superType, List<Type> interfaceTypes) {

        System.out.println("StartType: " + line + " name=" + name + " type=" + superType);
        
        final String className = (type == null ? qualifier.getPackage() : type.getName() + "$") + name;
        boolean isInterface = false;
        final List<ClassInfo> interfaceClasses = new ArrayList<ClassInfo>();
        for (Type interfaceType : interfaceTypes) {            
            interfaceClasses.add(toClassInfo(interfaceType));
        }
        final ClassInfo info = new ClassInfo(className, isInterface, toClassInfo(superType), interfaceClasses, src);
