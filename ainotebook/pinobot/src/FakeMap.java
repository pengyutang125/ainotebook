...
public class FakeMap {
    ...
    public static int OpponentY() {
        return (int) random.nextInt(height-1);
    }
    ...
    public static void MakeMove(String direction) {
        if (direction.length() <= 0) {
            System.err.println("FATAL ERROR: empty direction string. You " + "must specify a valid direction in which "
                    + "to move.");
            System.exit(1);
        }
    ...
    public static void myInit() {
        ...
        myLocation = new java.awt.Point(0, 0);
        opponentLocation = new java.awt.Point(20, 20);
    ...
    public static void Initialize() {
        ...
        try {
            ...
            System.err.println("FATAL ERROR: invalid map dimensions: " + firstLine);
            System.err.flush();
            System.exit(1);
        }
        walls = new boolean[width][height];
        ...
        while (y < height) {
            ...
            case '\n':
                if (x != width) {
                    System.err.println("Invalid line length: " + x + "(line " + y + ")");
                    System.exit(1);
                }
                ++y;
                x = 0;
            ...
            case '1':
                if (foundMyLocation) {
                    System.err.println("FATAL ERROR: found two locations " + "for player "
                            + "1 in the map! First location is (" + myLocation.getX() + "," + myLocation.getY()
            ...
            case '2':
                if (foundHisLocation) {
                    System.err.println("FATAL ERROR: found two locations for player "
                            + "2 in the map! First location is (" + opponentLocation.getX() + ","
                            + opponentLocation.getY() + "), second location " + "is (" + x + "," + y + ").");
                    System.exit(1);
                }
                walls[x][y] = true;
                ...
                break;
            default:
                System.err.println("FATAL ERROR: invalid character received. " + "ASCII value = " + c);
                System.exit(1);
            }
            ++x;
            ++numSpacesRead;
        ...
        if (numSpacesRead != width * height) {
            System.err.println("FATAL ERROR: wrong number of spaces in the map. " + "Should be " + (width * height)
