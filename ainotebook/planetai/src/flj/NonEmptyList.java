 ...
 * @version 2.20<br>
 *          <ul>
 *          <li>$LastChangedRevision: 180 $</li>
 *          <li>$LastChangedDate: 2009-06-28 09:59:09 +1000 (Sun, 28 Jun 2009) $</li>
 *          </ul>
 */
public final class NonEmptyList<A> implements Iterable<A> {
  ...
  public NonEmptyList<NonEmptyList<A>> sublists() {
    return fromList(
        Option.somes(toList().toStream().substreams()
  ...
  public NonEmptyList<NonEmptyList<A>> tails() {
    return fromList(Option.somes(toList().tails().map(new F<List<A>, Option<NonEmptyList<A>>>() {
      public Option<NonEmptyList<A>> f(final List<A> list) {
        return fromList(list);
      }
    }))).some();
  ...
  public static <A> Option<NonEmptyList<A>> fromList(final List<A> as) {
    return as.isEmpty() ?
           ...
           Option.some(nel(as.head(), as.tail()));
  }
}
