...
public abstract class Either<A, B> {
  ...
  public final class LeftProjection<A, B> implements Iterable<A> {
    ...
    public List<A> toList() {
      return isLeft() ? List.single(value()) : List.<A>nil();
    }
    ...
    public Option<A> toOption() {
      return isLeft() ? Option.some(value()) : Option.<A>none();
    }
  ...
  public final class RightProjection<A, B> implements Iterable<B> {
    ...
    public List<B> toList() {
      return isRight() ? List.single(value()) : List.<B>nil();
    }
    ...
    public Option<B> toOption() {
      return isRight() ? Option.some(value()) : Option.<B>none();
    }
  ...
  public static <A, B> Either<A, B> joinLeft(final Either<Either<A, B>, B> e) {
    final F<Either<A, B>, Either<A, B>> id = Function.identity();
    ...
    return a.isEmpty() ?
           ...
           a.head().left().bind(new F<A, Either<List<A>, X>>() {
             public Either<List<A>, X> f(final A aa) {
               return sequenceLeft(a.tail()).left().map(List.cons_(aa));
             }
           });
  }
    ...
    return a.isEmpty() ?
           ...
           a.head().right().bind(new F<B, Either<X, List<B>>>() {
             public Either<X, List<B>> f(final B bb) {
               return sequenceRight(a.tail()).right().map(List.cons_(bb));
             }
           });
  }
