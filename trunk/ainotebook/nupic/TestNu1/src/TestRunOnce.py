...
from nupic.network import CreateRuntimeNetwork
...
class BitwormData(netexplorer.DataInterface):
    ...
    def createBitworm(self, wormType, pos, length, inputSize):
        ...
        bit = 0
        input = []
        for i in range(0, pos): input.append(self.getBit(0))
        if wormType == 'solid':
                self.categories.append(0)
                for i in range (pos, pos+length): input.append(self.getBit(1))
        ...
        for i in range (pos+length, inputSize): input.append(self.getBit(0))            
    ...
    def appendBlank(self):
        ...
        size = self['inputSize']
        blank = []
        for i in range(0,size): blank.append(0)
    ...
    def createDataNoTemporalCorrelation(self):
        ...
        for i in range(0, self['numSequencesPerBitwormType']):
    ...
    def createData(self):
        ...
        for i in range(0, self['numSequencesPerBitwormType']):
          ...
          for wormType in ['solid','textured']:
            ...
            for j in range(0, self['sequenceLength']):
              self.createBitworm(wormType, pos, length, size)
              if pos+length >= size:
                increment = -1*increment
...
def runApp():
    ...
    generateReport(trainedNetwork, trainingResults, trainingCategories, testResults, testCategories, reportFile)    
