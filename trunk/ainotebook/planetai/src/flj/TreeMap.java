...
public final class TreeMap<K, V> implements Iterable<P2<K, V>> {
  ...
  public Iterator<P2<K, V>> iterator() {  
      ...
      return (Iterator<P2<K, V>>) IterableW.join(
              tree.toStream()
  ...
  public P2<Boolean, TreeMap<K, V>> update(final K k, final F<V, V> f) {
    final P2<Boolean, Set<P2<K, Option<V>>>> up =
        tree.update(P.p(k, Option.<V>none()), P2.<K, Option<V>, Option<V>>map2_(Option.<V, V>map().f(f)));
  ...
  public P3<Set<V>, Option<V>, Set<V>> split(final K k) {
    final FW<Set<P2<K, Option<V>>>, Set<V>> getSome = FW.$(Option.<V>fromSome()).o(P2.<K, Option<V>>__2())
        .mapSet(tree.ord().comap(F2W.$$(P.<K, Option<V>>p2()).f(k).<V>o(Option.<V>some_())));
    return tree.split(P.p(k, Option.<V>none())).map1(getSome).map3(getSome)
        .map2(FW.$(Option.<V>join()).o(FW.$(P2.<K, Option<V>>__2()).mapOption()));
  }
  ...
  public <W> TreeMap<K, W> map(final F<V, W> f) {
    final F<P2<K, Option<V>>, P2<K, Option<W>>> g = P2.map2_(FW.$(f).mapOption());
    final FW<K, P2<K, Option<V>>> coord = F2W.$$(P.<K, Option<V>>p2()).flip().f(Option.<V>none());
