...
public class Bot extends SimpleTagSupport
{
    ...
    {
        this.name = value;
        final BotAccess botAccess = ((BotAccess) getJspContext().getAttribute("bot", PageContext.SESSION_SCOPE));
        if (botAccess != null)
        {
            this.bot = botAccess.getBot();
            System.out.println("[trace]" + this.bot);
        } else {
			System.out.println("[trace] null bot access");
		} // End of the if - else //
    }
